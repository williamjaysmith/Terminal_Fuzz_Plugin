...
void TerminalCircuit::prepare(double sampleRate, int samplesPerBlock) {
...
void TerminalCircuit::processBlock(juce::AudioBuffer<float>& buffer,
    ...
    for (int channel = 0; channel < numChannels && channel < Constants::MAX_CHANNELS; ++channel) {
        ...
        for (int sample = 0; sample < numSamples; ++sample) {
            ...
            // Track input level for LED meter (only on first channel)
            if (channel == 0) {
...
float TerminalCircuit::processSample(float sample, int channel,
    ...
    if (cv.bypass_mode) {
        ...
        if (bypass_debug <= 3) {
            ...
            if (debug_file) {
                ...
                fprintf(debug_file, "Input: %.6f, Level: %.6f, Output: %.6f\n", 
                       sample, levelAmount, sample * levelAmount);
                fclose(debug_file);
            }
        }
    ...
    if (debug_count <= 5) {
        ...
        if (debug_file) {
            ...
            fprintf(debug_file, "Level Amount: %.6f\n", levelAmount);
            fprintf(debug_file, "Level Amount NaN? %s\n", std::isnan(levelAmount) ? "YES - PROBLEM!" : "NO");
